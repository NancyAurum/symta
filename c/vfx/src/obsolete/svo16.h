...
/*
TODO:
- Since mesh terms always have c=1, we can iterate on them, instead of iterating
  on empty terms, and then checking the neigbors for empty.
...
/*
#define OT_FORK      0x8000
#define IS_FORK(n)   ((n)&OT_FORK)
#define IS_TERM(n)   (!IS_FORK(n))
#define OT_FPTR(n)   ((n)&0x7FFF)
*/

#define OT_FORK      0xEFFF
#define IS_FORK(n)   ((n)>=OT_FORK)
#define IS_TERM(n)   ((n)<OT_FORK)
#define OT_FPTR(n)   ((n)-OT_FORK)


//first two are for the empty and the filler color.
#define OT_PREDEF    2
#define OT_EMPTY     0x0000
#define OT_CLAY      0x0001
...
INLINE void ot_t.clear(U32 color) {
  ...
  OTP *octs = topo.add->octs; //root node
...
INLINE attr_t *ot_t.sattr(U16 *oct) {
  attr_t *attr;
...
INLINE attr_t *ot_t.set1(U32 x, U32 y, U32 z, int erase) {
  U32 c = this->c;
  nodes_t *topo = &this->topo;
  U32 ptr = 0;
  OTP *octs = topo.ref(ptr)->octs;
...
static void ot_mesh_terms(ot_t *this, SVOTerms *terms
                      ,U32 nptr, U32 c
                      ,U32 x, U32 y, U32 z)
{
  OTP *octs = this->topo.ref(nptr)->octs;
  for (int i = 0; i < 8; i++) {
    U32 ptr = octs[i];
    U32 ox=x,oy=y,oz=z;
    if (i&0x1) ox += c;
    ...
    if (i&0x4) oz += c;
    if (IS_FORK(ptr)) {
      ot_mesh_terms(this, terms, OT_FPTR(ptr), c>>1, ox, oy, oz);
    } else {
      if (c>1) continue;
      attr_t *attr = this->attr.ref(ptr);
...
static void ot_terms(ot_t *this, SVOTerms *terms
                    ,U32 nptr, U32 c
                    ,U32 x, U32 y, U32 z)
{
  OTP *octs = this->topo.ref(nptr)->octs;
  for (int i = 0; i < 8; i++) {
    ...
    if (i&0x4) oz += c;
    if (IS_FORK(ptr)) {
      ot_terms(this, terms, OT_FPTR(ptr), c>>1, ox, oy, oz);
    } else {
      term_t *t = terms.add;
      t->o.x = ox;
      t->o.y = oy;
...
static U32 ot_optimize(nodes_t *RESTRICT topo, U32 nptr) {
  OTP *octs = topo.ref(nptr)->octs;
  for(int oct = 0; oct < 8; oct++) {
    U32 ptr = octs[oct];
    if (IS_TERM(ptr)) continue;
    U32 r = ot_optimize(topo, OT_FPTR(ptr));
    if (r) octs[oct] = r;
  }
  U32 v = octs[0];
...
INLINE void ot_t.term_set(term_t *t, U32 color) {
  OTP *octs = this->topo.ref(t->node)->octs;
  this.sattr(&octs[t->oct])->color = color;
}

INLINE void ot_t.term_set_mesh(term_t *t, U32 color) {
  OTP *octs = this->topo.ref(t->node)->octs;
...
INLINE void ot_t.term_split(term_t *t, SVOTerms *terms) {
  ...
  topo.ref(t->node)->octs[t->oct] = topo->used+OT_FORK;
...
INLINE void ot_t.set_clay_color(U32 color) {
