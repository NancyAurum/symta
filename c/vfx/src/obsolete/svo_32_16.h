...
#define OT_FORK16      0x8000
#define IS_FORK16(n)   ((n)&OT_FORK16)
#define IS_TERM16(n)   (!IS_FORK16(n))
#define OT_FPTR16(n)   ((n)&0x7FFF)

typedef struct {
  U16 octs[8]; //octants (OT_FORK16 bit set means branching)
} node16_t;

CXLst(nodes16_t,node16_t)

//32x32x32 segment, inside which 16-bit pointers are enough.
typedef struct seg_t {
  nodes16_t topo; //topology: nodes and links into atrs (list of U16)
...
INLINE attr_t *seg_t.sattr(ot_t *parent, U16 *oct) {
  ...
  if (ptr >= OT_PREDEF) {
    ...
    attr_t stub_attr = parent->predef[ptr];
    if (this->fattr) {
      U32 aptr = this->fattr;
      *oct = aptr;
      attr = atrs.ref(aptr);
...
INLINE attr_t *seg_t.get(U32 c, ot_t *parent, U32 x, U32 y, U32 z) {
  ...
  for(;;) {
    ...
    ptr = octs[oct];
    if (IS_TERM16(ptr)) {
      if (ptr < OT_PREDEF) return &parent->predef[ptr];
      return this->atrs.ref(ptr);
    }
    c>>=1;
    ptr = OT_FPTR16(ptr);
  }
...
INLINE attr_t *seg_t.set(ot_t *parent, U32 c, U32 x, U32 y, U32 z, int type) {
  ...
  for (;;) {
    ...
    U16 ptr = octs[oct];
    if (IS_FORK16(ptr)) {
      octs = topo.ref(OT_FPTR16(ptr))->octs;
      continue;
    }
    parent->nmods++;
    octs[oct] = topo.used|OT_FORK16; //after `add` the octs link will be invalid
    octs = topo.add->octs;
...
INLINE void ot_terms16(seg_t *this, U32 segid, SVOTerms *terms
                    ,U32 nptr, U32 c
                    ,U32 x, U32 y, U32 z
                    ,int type)
{
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (i&0x4) oz += c;
    if (IS_FORK16(ptr)) {
      ot_terms16(this, segid, terms, OT_FPTR16(ptr), c>>1, ox, oy, oz, type);
    } else {
      if (type==TERM_MESH) {
        if (c>1) continue;
...
{
    if (c < 32) {
    U32 segid = this.xyz2seg(x,y,z); //FIXME: use nptr instead
    seg_t *seg = this->segs + segid;
    ot_terms16(seg, segid, terms, 0, c, x, y, z, type);
    return;
  }
...
INLINE U32 ot_t.exists(term_t *t) {
  if (t->seg == OT_NOSEG) return this->topo.ref(t->node)->octs[t->oct];
  return this->segs[t->seg].topo.ref(t->node)->octs[t->oct];
}

INLINE U32 ot_t.term_color(term_t *t) {
  if (t->seg == OT_NOSEG) {
    U32 ptr = this->topo.ref(t->node)->octs[t->oct];
    return this->predef[ptr].color;
  }
  seg_t *seg = this->segs + t->seg;
  U16 ptr = seg->topo.ref(t->node)->octs[t->oct];
  if (ptr < OT_PREDEF) return this->predef[ptr].color;
  return seg->atrs.ref(ptr)->color;
}
...
INLINE void ot_t.term_split16(term_t *t, SVOTerms *terms) {
  ...
  if (c2 != 16) {
    ...
    *oct = node|OT_FORK16;
    ...
    U32 *oct = &this->topo.ref(t->node)->octs[t->oct];
...
static U32 ot_optimize(U32 clay_color, atrs_t *atrs
                      ,nodes16_t *RESTRICT topo, U16 nptr) {
  U16 *octs = topo.ref(nptr)->octs;
  for(int oct = 0; oct < 8; oct++) {
    U16 ptr = octs[oct];
    if (IS_TERM16(ptr)) {
      attr_t *attr = atrs.ref(ptr);
      if (clay_color == attr->color) {
        octs[oct] = OT_CLAY;
      }
      continue;
    }
    U32 r = ot_optimize(clay_color, atrs, topo, OT_FPTR16(ptr));
...
INLINE void ot_t.optimize {
  ...
  for (int i = 0; i < n; i++) {
    seg_t *seg = segs+i;
    ot_optimize(this->predef[OT_CLAY].color, &seg->atrs, &seg->topo, 0);
  }
}

static U16 ot_realloc(nodes16_t *RESTRICT otopo, atrs_t *RESTRICT oatrs
                          ,nodes16_t *RESTRICT itopo, atrs_t *RESTRICT iatrs
                          ,U16 nptr) {
  ...
  for(int oct = 0; oct < 8; oct++) {
    U16 ptr = octs[oct];
    if (IS_FORK16(ptr)) {
      ptr = ot_realloc(otopo, oatrs, itopo, iatrs, OT_FPTR16(ptr))|OT_FORK16;
...
INLINE U32 ot_t.clay_color {
  return this->predef[OT_CLAY].color;
}

INLINE void ot_t."clay_color="(U32 color) {
  this->predef[OT_CLAY].color = color;
}
...
void ot_t.raycastS(svo_res_t *res, svo_stack_t *stack, svo_ray_t ray) {
  ...
  while (scale < OT_MAX_SCALE) {
    ...
    if (IS16BIT_SCALE(scale)) {
      U32 oct = ((node16_t*)node)->octs[oct_index];
      is_filled = oct;
      is_term = IS_TERM16(oct);
    } else {
    ...
    if (is_filled && t_min <= t_max) {
      ...
      if (t_min <= tv_max) {
        ...
        //move to the child
        if (IS16BIT_SCALE(scale)) {
          int oct = ((node16_t*)node)->octs[oct_index];
          U32 ptr = OT_FPTR16(oct);
          node = stopo.ref(ptr);
        } else {
          int oct = ((node_t*)node)->octs[oct_index];
          U32 ptr = OT_FPTR(oct);
...
INLINE attr_t *ot_t.oct_attr(void *node, seg_t *seg, int oct) {
  if (seg) {
    U16 ptr = ((node16_t*)node)->octs[oct];
    if (ptr < OT_PREDEF) return &this->predef[ptr];
    return seg->atrs.ref(ptr);
  }
  U32 ptr = ((node_t*)node)->octs[oct];
  return &this->predef[ptr];
}
