...
#define __gensym1(a, b) _gensym##a##b
#define gensym(name) PRFX(name,_,__gensym1(__COUNTER__,__LINE__))

#define LET(expr) \
  for (int gensym(i)=0; gensym(i)<1;) for (expr; gensym(i)<1; gensym(i)=1)

//https://en.cppreference.com/w/c/thread/thread_local
//https://stackoverflow.com/questions/32245103/how-is-gccs-thread-implemented
//#define thread_local _Thread_local

//these make switch easier to use
#define CASE(k) switch(k) { {
#define GOT(x) } break; case x: {
#define ESAC } break;}

#define times(i,n) for (int i = 0, times_n_=(n); i < times_n_; i++)
...
#define cat(...) _hidden_cat_(__VA_ARGS__, (void*)0)
