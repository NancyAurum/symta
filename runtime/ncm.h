/*
  ...
  macros, default and variadic args, body-block args ({...}),
  #if/#elif/#else/#endif with compile-time integer arithmetic,
  #include via `#:name`, scopes via `#{ ... #}`, and a
  printf-style `#("fmt" args...)` formatter.
...
#define save_position_state U32 saved_row, saved_col, saved_pcol; U8 *saved_ptr;
#define save_position() \
  saved_row = filep->row; \
  saved_col = filep->col; \
  saved_pcol = filep->pcol; \
  saved_ptr = filep->ptr;
...
static char *call_macro2(ncm_state *this, macro_t *m, char *expr, char **endp);

static S32 eval_expr_standalone(ncm_state *this, char *expr, char **endp) {
  save_position_state
  save_position();
  filep->ptr = (U8*)expr; //FIXME: huge kludge
  S32 result = eval_expr(this);
  if (endp) *endp = (char*)filep->ptr;
  load_position();
  return result;
}
...
static char *inject_vars(ncm_state *this, char *mname, char *body
  ...
  while (*b) {
    ...
    if (c == MCH) {
      if (b[1] == '[') {
        b += 2;
        S32 result = eval_expr_standalone(this, b, &b);
        if (*b  != ']') {
...
static void call_macro(ncm_state *this, macro_t *m) {
  char *args = NULL;
  if (m->nargs != -1) {
    if (next_char() != '(') {
...
static char *call_macro2(ncm_state *this, macro_t *m, char *expr, char **endp) {
  ...
  abody_t sabody = this->abody;
  this->abody.ptr = abody;
  char *expanded = macroexpand(this, m, args, ME_STANDALONE);
  this->abody = sabody;
  if (args) arrfree(args);
  if (abody) arrfree(abody);
  return expanded;
...
symbol:;
    ...
    if (m && m->nargs != -1) {
      char *expanded = call_macro2(this, m, 0, 0);
      arrfree(name);
      S32 result = eval_expr_standalone(this, expanded, 0);
      arrfree(expanded);
      return result;
    }
    if (m) s = m->body;
